...
from utlx.ctypes import POINTER
...
if TYPE_CHECKING:
    transfer_cb_fn_type: TypeAlias = Callable[[POINTER[transfer]], None]
...
if TYPE_CHECKING:
    log_cb_type: TypeAlias = Callable[[POINTER[context], log_level, ct.c_char_p], None]
...
# static inline
def control_transfer_get_data(transfer: POINTER[transfer]) -> POINTER[ct.c_ubyte]:
    transf = transfer[0]
...
# static inline
def control_transfer_get_setup(transfer: POINTER[transfer]) -> POINTER[control_setup]:
    transf = transfer[0]
...
# static inline
def fill_control_setup(buffer: POINTER[ct.c_ubyte], bmRequestType: ct.c_uint8,
...
# static inline
def fill_control_transfer(transfer: POINTER[transfer], dev_handle: POINTER[device_handle],
                          buffer: POINTER[ct.c_ubyte], callback: transfer_cb_fn_type,
...
# static inline
def fill_bulk_transfer(transfer: POINTER[transfer], dev_handle: POINTER[device_handle],
                       endpoint: ct.c_ubyte, buffer: POINTER[ct.c_ubyte], length: ct.c_int,
...
# static inline
def fill_bulk_stream_transfer(transfer: POINTER[transfer],
                              dev_handle: POINTER[device_handle], endpoint: ct.c_ubyte,
                              stream_id: ct.c_uint32, buffer: POINTER[ct.c_ubyte],
...
# static inline
def fill_interrupt_transfer(transfer: POINTER[transfer], dev_handle: POINTER[device_handle],
                            endpoint: ct.c_ubyte, buffer: POINTER[ct.c_ubyte], length: ct.c_int,
                            callback: transfer_cb_fn_type, user_data: ct.c_void_p,
                            timeout: ct.c_uint) -> None:
    transf = transfer[0]
...
# static inline
def fill_iso_transfer(transfer: POINTER[transfer], dev_handle: POINTER[device_handle],
                      endpoint: ct.c_ubyte, buffer: POINTER[ct.c_ubyte], length: ct.c_int,
...
# static inline
def set_iso_packet_lengths(transfer: POINTER[transfer], length: ct.c_uint) -> None:
    transf = transfer[0]
...
# static inline
def get_iso_packet_buffer(transfer: POINTER[transfer],
                          packet: ct.c_uint) -> POINTER[ct.c_ubyte]:
...
# static inline
def get_iso_packet_buffer_simple(transfer: POINTER[transfer],
                                 packet: ct.c_uint) -> POINTER[ct.c_ubyte]:
    packet_no: int = packet.value
...
# static inline
def get_descriptor(dev_handle: POINTER[device_handle], desc_type: ct.c_uint8,
                   desc_index: ct.c_uint8, data: POINTER[ct.c_ubyte],
...
# static inline
def get_string_descriptor(dev_handle: POINTER[device_handle],
                          desc_index: ct.c_uint8, langid: ct.c_uint16,
                          data: POINTER[ct.c_ubyte], length: ct.c_int) -> ct.c_int:
    return typing.cast(ct.c_int,
           control_transfer(dev_handle,
                            LIBUSB_ENDPOINT_IN, LIBUSB_REQUEST_GET_DESCRIPTOR,
...
def set_option(ctx: POINTER[context], option: option,

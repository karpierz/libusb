...
static libusb_testlib_result test_get_device_list(libusb_testlib_ctx * tctx)
{
    ...
    for (i = 0; i < 1000; ++i) {
        ...
        ssize_t list_size = libusb.get_device_list(ctx, &device_list);
...
#define LIST_COUNT 100
    ...
    for i in range(LIST_COUNT):
        ssize_t list_size = libusb.get_device_list(ctx, &(device_lists[i]));
    ...
    for i in range(LIST_COUNT):
        if (device_lists[i]) {
            libusb.free_device_list(device_lists[i], 1);
            device_lists[i] = NULL;
        }
    }
...
static libusb_testlib_result test_default_context_change(libusb_testlib_ctx * tctx)
{
    ...
    for i in range(100):
        ...
        libusb.set_debug(NULL, LIBUSB_LOG_LEVEL_DEBUG)
        libusb.set_debug(ctx,  LIBUSB_LOG_LEVEL_DEBUG)
...
def main():
    ...
    return libusb_testlib_run_tests(sys.argv, tests)
